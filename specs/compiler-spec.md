...
Local/catalog references and recursive schemas are allowed when the pinned native validator supports them. Schema documents, values, catalog traversal, and derived indexes use finite size/depth/work limits; regex compilation and matching use the bounds supported by that validator. The baseline native schema backend does not promise a general whole-validation instruction counter, recursive evaluation-depth counter, or hard in-process deadline. Its actual supported limits must be reported by the pinned profile, and unsupported enforcement requirements must be rejected rather than simulated. An operational watchdog stop is an engine/runtime failure, never a cached logical false. Deterministic work accounting for HTLK's own Rust expression evaluator remains separate. There is no requirement to solve general schema subtyping. Unrepresentable refinements remain attached to a `schema` type and are checked on actual values.
