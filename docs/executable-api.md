# Executable model, analysis and runtime API reference
...
Scope, node, edge, and operation records assemble those declarations into bounded
representations; local names/use roles and full graph semantics are analyzer work.
...
records. `CanonicalDocument` checks representation and record identities. The
...
use htlk_executable::{CanonicalDocument, DocumentFields,
...
the runtime `CallbackInvocation::invoke_callback` extension trait with ordinary value arguments to enforce
...
The analyzer receives a context derived from the owning graph location, never
from an untrusted context field. Model construction/serialization preserves
representable references so the analyzer can report located semantic failures:
...
The model normalizes parts and checks parameter representation. The analyzer's
`check_prompt_template` checks exact declaration/slot name coverage, and this
check is included in expression environments and document linkage. Runtime also
applies it when admitting a frame template or rendering from a custom context.
...
The analyzer checks that edges address declared local nodes/ports and boundary
destinations even when guards are false, and resolves loop initializer inputs.

During analysis, `ScopeContext::Ordinary` requires empty carried declarations and disallows carried/
...
Closed-record layouts are model invariants. Local endpoint membership, reference
availability and use-role legality are analyzer checks alongside names/types,
dependency cycles, observability, binding coverage and interface compatibility.
Host profile matching and execution belong to runtime. Model constructors perform
no semantic admission, execution or persistence.
...
unique in the model. The analyzer checks that every variable in a parameter/result (including
...
exact heap measurements. Analyzer signature-variable resolution follows model size bounds.
...
use htlk_executable::cbor::Limits;
use htlk_executable::{JsonDocument, JsonPointer};
use htlk_analyzer::SchemaLocations;
...
use htlk_executable::cbor::Limits;
use htlk_executable::{JsonDocument, JsonPointer};
use htlk_analyzer::SchemaResources;
...
use htlk_executable::cbor::Limits;
use htlk_executable::{JsonDocument, JsonPointer};
use htlk_analyzer::SchemaCatalog;
...
the analyzer's `DocumentAnalysis::schema_catalog` extension; unused entries are rejected rather than pruned.
...
use htlk_executable::cbor::Limits;
use htlk_executable::JsonDocument;
use htlk_analyzer::{SchemaCatalog,SchemaReferenceKind};
...
use htlk_executable::cbor::Limits;
use htlk_executable::JsonDocument;
use htlk_analyzer::embedded_schema_base;
...
After record assembly or decoding, import the analyzer's `DocumentAnalysis` trait
and call `document.schema_catalog(&Limits)`
...
`LinkageError::Schema` preserves schema/resource/reference failures, and unused
...
`htlk-analyzer/assets/`. Native validators compile this trusted data once; no schema is fetched
while checking submitted descriptors. Protocol extension fields remain preserved.

The analyzer's `DocumentAnalysis::validate_mcp_descriptors` composes this with binding
...
use htlk_executable::cbor::Limits;
use htlk_executable::JsonDocument;
use htlk_analyzer::{NativeSchemaOptions,NativeSchemas,SchemaCatalog};
...
The analyzer's `DocumentAnalysis::native_schemas` composes the executable schema-catalog stage
...
Model assembly checks record hashes, raw JCS identities and library keys against
supplied implementation identities. Analyzer linkage checks root/policy existence,
scope-definition acyclicity and exact scope/template/binding/library reachability.
Scope roles are derived from use sites during analysis; a shared definition must satisfy every
...
descriptor. Analyzer linkage canonicalizes each extracted object under the JSON limits
...
Analyzer linkage enforces `maximum_scope_depth` and `maximum_expanded_nodes`
from the linked policy. `LinkedDocument::structural_summary()` and
`AnalyzedDocument::structural_summary()` return an
...
`LinkageError::StructuralLimitExceeded` with `StructuralLimit::ScopeDepth` or
...
Registration and authorization remain runtime work. `DocumentError` and `LinkageError` preserve
...
Decode first applies `htlk_executable::cbor::decode` with the supplied `Limits`. Schema
...
let limits = Limits::default();
let encoded = htlk_executable::cbor::encode(&value, &limits)?;
