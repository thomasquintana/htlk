...
impl NativeSchemas {
    ...
    /// Validates bounded JSON against a prescribed immutable schema root.
    ///
    /// # Errors
    /// Returns missing root, input limits or native engine failures.
    ...
    /// Whether a digest identifies a compiled prescribed schema root.
    pub fn has_schema_type(&self, schema: &Digest) -> bool {
    ...
    /// Writes native evaluation annotations for an already valid root instance.
    /// Runtime integrations supply a bounded writer and meter value conversion.
    ///
    /// # Errors
    /// Returns missing root, invalid instance, input limits or output failures.
    pub fn projection_evaluation(
        &self,
        schema: &Digest,
        instance: &JsonDocument,
