...
/// # Examples
/// ```
/// use htlk_executable::cbor::{decode, Limits, Value};
...
impl<'a> Decoder<'a> {
    ...
    fn argument(&mut self, additional: u8) -> Result<u64, Error> {
        ...
        // length diagnostics even on targets whose usize is narrower than u64.
        let mut bytes = [0; 9];
        bytes[0] = additional;
        bytes[1..1 + width].copy_from_slice(self.take(width)?);
        let Header::Positive(argument) = HeaderDecoder::from(&bytes[..1 + width])
            .pull()
            .expect("complete unsigned header")
        else {
    ...
    fn simple(&mut self, additional: u8) -> Result<Value, Error> {
        let value = match additional {
            ...
            25..=27 => {
                ...
                let mut decoder = HeaderDecoder::from(&self.input[start..]);
                let Header::Float(value) = decoder
                    .pull()
                    .map_err(|_| Error::new(ErrorKind::UnexpectedEnd).at(self.input.len()))?
                else {
                    unreachable!("float prefix")
                };
                self.position = start + decoder.offset();
                value
            }
            _ => return Err(Error::new(ErrorKind::UnsupportedType)),
        };
