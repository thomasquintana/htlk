...
fn matches_primitive(
    ...
    match p {
        ...
        P::Error => {
            let shape = crate::types::builtin_record_type(p, meter.program_limits())?
...
fn projected_types(
    ...
    let child = match (ty.kind(), step) {
        ...
        (K::Primitive(p), _) => {
            if let Some(record) = crate::types::builtin_record_type(*p, meter.program_limits())? {
...
fn select<'a>(
    ...
    match ty.kind() {
        ...
        K::Primitive(p) => {
            let Some(record) = crate::types::builtin_record_type(*p, meter.program_limits())?
            else {
                return Ok(Selection::Invalid);
            };
            select(
