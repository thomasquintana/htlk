...
use htlk_executable::{
    ...
    ExpressionContext as C, ExpressionKind as E, NativeSchemaOptions, NativeSchemas, Node,
    NodeFields, Operation, Port, PortTable, PrimitiveType as P, ScalarLiteral as L, SchemaCatalog,
    Scope, ScopeContext as Role, ScopeFields, ScopeVerificationErrorKind as Error, ValueReference,
    ValueType, WaitVertex, verify_scope_graph,
};
...
fn verify(
    fields: ScopeFields,
) -> Result<htlk_executable::ScopeGraphPlan, htlk_executable::ScopeVerificationError> {
...
fn optional_sources_can_feed_required_destinations_without_expression_presence_errors() {
    ...
    assert!(
        !boundary
            ...
            .runtime_checks()
            .iter()
            .any(|check| matches!(check.kind, htlk_executable::RuntimeTypeCheckKind::Present))
    );
}
...
fn expression_failures_report_canonical_subtrees_without_literal_contents() {
    ...
    assert_eq!(
        error.site,
        Some(htlk_executable::ExpressionSite::NodeGuard(
            "a".parse().unwrap()
        ))
    );
