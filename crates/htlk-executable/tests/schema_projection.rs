...
use htlk_executable::{
    EvaluationError as Error, EvaluationValue as V, EvaluatorLimits, JsonDocument,
    NativeSchemaOptions, NativeSchemas, PathStep, SchemaCatalog, SchemaProjection, digest::Digest,
    embedded_schema_base,
};
...
fn checked_expression_pins_schemas_and_preserves_nested_optional_results() {
    use htlk_executable::{
        ...
        ExpressionKind as E, ExpressionTypeEnvironment, Port, TypeContext, ValueReference,
...
fn schema_roots_nested_in_structural_records_keep_their_context() {
    use htlk_executable::{
        Port, TypeContext, ValueType as T, ValueTypeKind as K, project_typed_value,
    };
    let (schemas, id) =
...
fn known_schema_families_reject_disjoint_boundaries_without_numeric_coercion() {
    use htlk_executable::{
        CheckedExpression, Expression, ExpressionContext as C, ExpressionKind as E,
        ExpressionTypeEnvironment, ExpressionTypeError, Port, PrimitiveType as P, TypeContext,
        ValueReference, ValueType as T, ValueTypeKind as K,
    };
    let (schemas, id) = compile(
...
fn schema_context_survives_record_materialization_without_reinvoking_native_functions() {
    use htlk_executable::{
        CheckedExpression, EvaluationArgument, EvaluationContext, EvaluationMeter,
        EvaluationOutcome, Expression, ExpressionContext as C, ExpressionKind as E,
        ExpressionTypeEnvironment, FunctionId, FunctionSignature, Identifier, Library, Port,
        PromptTemplate, TypeContext, ValueReference, ValueType as T, ValueTypeKind as K,
    };
    use std::cell::Cell;
    struct Context {
        calls: Cell<usize>,
