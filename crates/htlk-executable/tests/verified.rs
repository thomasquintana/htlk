...
use htlk_executable::{
    ...
    EvaluationError, EvaluationFrame, EvaluationValue, EvaluatorLimits, ExecutableEnvelope,
    ExecutableVerificationError as Error, ExecutionLimits, ExecutionProfile, Expression,
    ExpressionContext as C, ExpressionKind as E, ExpressionSite, NativeRegistry,
    NativeRegistryError, Node, NodeFields, Operation, PolicyDocument, PolicyFields, Port,
    PortTable, PrimitiveType as P, ScalarLiteral as L, Scope, ScopeContext as Role, ScopeFields,
    ScopeUse, ScopeVerificationErrorKind, ValueReference, ValueType, digest::Digest,
    native_profile, verify_executable,
};
...
fn valid_record_assembly_is_not_sufficient_for_graph_admission() {
    ...
    let Err(Error::Graph(error)) = result else {
...
fn all_mcp_binding_kinds_pass_the_complete_offline_admission_boundary() {
    use htlk_executable::{
        ...
        TypeContext, ValueTypeKind, embedded_schema_base,
    };
    let limits = Limits::default();
    let registry = registry();
...
fn canonical_construction_order_is_irrelevant_and_unused_documents_are_rejected() {
    ...
    assert!(
        matches!(verify_executable(&bytes(extra),&registry,&limits),Err(Error::Document(error)) if matches!(*error,htlk_executable::DocumentError::UnreachableRecord(_)))
    );
}
